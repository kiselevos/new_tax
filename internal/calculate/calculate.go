// Package calculator реализует расчёт НДФЛ по правилам 2025 года.
//
// 📘 Логика налоговых баз (согласно разъяснениям ФНС России)
//
// Согласно п.6.2 ст.210 Налогового кодекса РФ и официальным письмам
// ФНС России (в т.ч. № БС-4-11/3051@ от 18.03.2021), доходы работника
// при расчёте НДФЛ делятся на две налоговые базы:
//
//  1. Основная налоговая база — заработная плата (включая районный
//     коэффициент). Облагается по прогрессивной шкале ставок 13–22%,
//     в зависимости от совокупного годового дохода.
//
//  2. Дополнительная налоговая база — северная надбавка. Облагается
//     отдельно по упрощённой шкале: 13% до 5 млн рублей и 15% свыше.
//
// ⚙️ Принцип расчёта в калькуляторе:
//
//  - Территориальный коэффициент увеличивает оклад и входит в основную
//    налоговую базу, облагаемую по прогрессивной шкале.
//
//  - Северная надбавка рассчитывается уже от суммы «оклад + районный
//    коэффициент» и облагается по упрощённой шкале (13/15%).
//
// Формулы:
//    grossWithTerritorial = grossSalary * territorialMultiplier / 100
//    northernAddition     = grossWithTerritorial * (northernCoefficient - 100) / 100
//
// Таким образом, порядок вычислений:
//
//    1. Рассчитать базовый доход с учётом территориального коэффициента.
//    2. Отдельно рассчитать северную надбавку от этой суммы.
//    3. Применить прогрессивную шкалу к основной базе и упрощённую шкалу
//       к северной надбавке.
//    4. Суммировать результаты, округлив налог по каждому типу базы
//       согласно п.6 ст.52 НК РФ (до полных рублей по правилу 50 копеек).
//
// Источники:
//   • Налоговый кодекс РФ, ст.210, ст.224, ст.52
//   • Письмо ФНС России № БС-4-11/3051@ от 18.03.2021
//   • Разъяснения на сайте nalog.gov.ru (раздел "НДФЛ с 2021 года — прогрессивная шкала")
