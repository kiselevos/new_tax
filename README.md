> ⚠️ **Экспериментальная ветка**
>
> Данная ветка является **учебной и экспериментальной**.
> 
> 1. Redis добавлен как дополнительный (overhead) слой кэширования для результатов налоговых расчётов.
> Кэширование работает по принципу cache-aside с TTL и не влияет на корректность работы сервиса — при отсутствии Redis расчёт выполняется напрямую.
> 2. Rate limiting реализован как через in-memory(golang.org/x/time/rate), так и через Redis lua script.
> При недоступности Redis автоматически происходит fallback на in-memory limiter.
>
> Ветка учебная и не планируется к слиянию в main.
